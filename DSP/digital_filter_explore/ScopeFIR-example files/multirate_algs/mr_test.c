...
{
    enum { TARGET_BLOCK_SIZE = 256 };
    
    double *p_inp_real, *p_inp_imag, *p_out_real, *p_out_imag, *p_Z_real,
           *p_Z_imag;

    int ii, num_inp, num_out, current_phase, block_num_out, block_size;
    ...
    /* for resampling, set current_phase to interp_factor so that resampler will
       load new data into the delay line prior to calculating any outputs */
    current_phase = interp_factor;  
            
    /* test the specified function */
    num_inp = num_out = 0;
    while (inp_size > 0) {
        block_size = ( TARGET_BLOCK_SIZE / decim_factor ) * decim_factor;    
        if (inp_size < block_size) {
            block_size = inp_size;
        }
        ...
        switch (multirate_function) {
            ...
            case DECIM_FUNCTION:
                ...
                decim_complex(decim_factor, H_size,
                              p_H, p_Z_real, p_Z_imag,
                              block_size,
                              p_inp_real + num_inp, p_inp_imag + num_inp,
                              p_out_real + num_out, p_out_imag + num_out,
                              &block_num_out);
                break;

            case INTERP_FUNCTION:
                assert(decim_factor == 1);
                interp_complex(interp_factor, num_phases,
                               p_H, p_Z_real, p_Z_imag,
                               block_size,
                               p_inp_real + num_inp, p_inp_imag + num_inp,
                               p_out_real + num_out, p_out_imag + num_out,
